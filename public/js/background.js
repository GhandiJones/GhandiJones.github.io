      ...
      scene.registerBeforeRender(function(){
        ...
        if(isDirectionalLerping){
          ...
          lerpCheck = (lerpCheck < 1) ? lerpCheck : 1;
          if(dirLerpCount % 3 === 0){
            currentSystem.direction1 = brighten(BABYLON.Vector3.Lerp(directionStore.orig.dir1, new BABYLON.Vector3(0, 0, 0), lerpCheck));
            currentSystem.direction2 = brighten(BABYLON.Vector3.Lerp(directionStore.orig.dir2, new BABYLON.Vector3(0, 0, -10), lerpCheck));
          } else {
          ...
          if(dirLerpTime / 1000 >= lerpRate){
              if(dirLerpCount % 3 === 0){
                ...
                currentSystem.radiusRange = 0.8;
              } else {
                currentSystem.noiseStrength = new BABYLON.Vector3(0, 0, -2);
                currentSystem.radiusRange = getRandomArbitrary(.3, 1);
              }
              dirLerpCount += 1;
              dirTime = 0;
      ...
      var starTexture = new BABYLON.Texture(".\\public\\imgs\\particle2.png", scene);
      ...
      // console.log(particleSystem.blendMode);
      /******* Emission Space ********/
      ...
      var sphereEmitter = particleSystem.createDirectedSphereEmitter(10, new BABYLON.Vector3(0, 0, 0), new BABYLON.Vector3(0, 0, -10));
                 ...
                 if (particle.age >= particle.lifeTime) { // Recycle
                      ...
                      if(d <= range){
                        ...
                        // systemBody.position.y += Math.sign(mousePos.y + Math.sign(systemBody.position.y)) * .01;
                          ...
                          //   totalPOS.x += rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          // }
                          // if(particle.position.x >= mousePos.x){
                          //   // particle.position.x += -rate * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          //   totalPOS.x += -rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          ...
                          //   totalPOS.y += rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          // }
                          // if(particle.position.y >= mousePos.y){
                          //   // particle.position.y += -rate * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          //   totalPOS.y += -rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 2;
                          ...
                          //   totalPOS.z += rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 10;
                          // }
                          // if(particle.position.z >= mousePos.z){
                          //   // particle.position.z += -rate * ((d + rangeEasing) / (range + rangeEasing)) * 10;
                          //   totalPOS.z += -rate;// * ((d + rangeEasing) / (range + rangeEasing)) * 10;
                        ...
                        // if (index === 0) {
                        //   console.log("Total ",totalPOS.normalize());
                        // }

                        //particle.position = totalPOS.normalize();

                        //particle.position.scaleInPlace(totalPOS.normalize());
